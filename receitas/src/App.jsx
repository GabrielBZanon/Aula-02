...
function updateReceita(id, data) {
  ...
  axios
    .put(`https://receitasapi-b-2025.vercel.app/receitas/${id}`, data)
    .then((response) => {
...
function App() {
  ...
  const [createReceitaData, setCreateReceitaData] = useState(null); // Estado para novo cadastro
  ...
  const handleCreateReceita = (newData) => {
    axios.post('https://receitasapi-b-2025.vercel.app/receitas', newData)
      .then((response) => {
  ...
  return (
    <>
      <header>
        <h1>Receitas</h1>
        <button onClick={() => setCreateReceitaData({})}>Cadastrar Receita</button>
      </header>
      <main className="card-container">
        {receitas.map((receita) => (
          <div className="card" key={receita.id}>
            ...
            <button onClick={() => setModalReceita(receita)}>
              Ver Receita
            </button>
            <button onClick={() => deleteReceita(receita.id)}>
              Deletar Receita
            </button>
            <button onClick={() => setEditReceitaData(receita)}>
              Editar Receita
            </button>
          </div>
        ))}
